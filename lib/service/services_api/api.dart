...
class MyAttendanceApi {
  ...
  Future<http.Response> callApi() async {
    ...
    try {
      ...
      log("MyAttendanceApi api called --------------> ${res.body}");
...
class RequestGatePass {
  ...
  Future<http.Response> callApi(BuildContext context) async {
    ...
    String url =
        "${Strings.baseUrl}saveoutpass?userid=$userid&studentid=$studentid&fromdate=$fromDate $fromTime&todate=$toDate $toTime&purpose=$reason&gardian=$gaurdian&otp=$otp&phone=$phone&applicationnumber=$applicationnumber";
    log(url);
    try {
